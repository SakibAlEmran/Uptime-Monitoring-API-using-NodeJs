/* 
    ...
    Descrition: Worker related files
...
worker.gatherAllChecks = () => {
    //get all the checks
    data.list('checks', (err1, cheks) => {
        if(!err1 && cheks && cheks.length > 0){
            cheks.forEach((check) => {
                //read the check data
...
worker.processCheckOutcome = (originalCheckData, checkOutCome) => {
    ...
    const alterWanted = !!(originalCheckData.lastChecked && originalCheckData.state !== state);
...
//send notofication sms to user if state changes
worker.alertUserToStatusChange = (newCheckData) => {
